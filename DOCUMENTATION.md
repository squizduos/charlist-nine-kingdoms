...
interface Character {
  ...
  experienceExplanation: string  // Заметка к полю "Опыт"
  health: number              // Урон (текущее значение, лимит = максимальное здоровье)
  healthExplanation: string   // Заметка к полю "Урон"
  injuries: Injuries          // Увечья
  wealth: number               // Богатство
  wealthExplanation: string    // Заметка к полю "Богатство"
  ...
  notes: string                     // VII. Заметки (Markdown-источник)
}
```
...
interface Mind {
  ...
  nightmares: number        // Кошмары (макс: 10 - permanentWill; не сбрасываются при смене permanentWill)
  bedlams: number           // Бедламы (макс: 10, фиксированный; растут только при превышении макс. кошмаров)
...
interface EditableParameter {
  name: string              // Название
  value: number             // Значение (диапазон зависит от секции, см. "Ограничения значений")
  explanation?: string      // Пояснение (опционально)
}
```

Используется для искусств, прав рождения, проклятий, связей, особенностей, артефактов и снаряжения (везде максимум 5), а также для расходников (максимум 9). Конкретный максимум задаётся пропом `max` компонента `DotStrip` в каждой секции, а не самим типом.

### Consumables

```typescript
interface Consumables {
  regular: EditableParameter[]  // 3 обычных расходника
  magical: EditableParameter[]  // 3 магических расходника
}
```
...
```typescript
// Текущий персонаж (с запасным пустым персонажем, если активная вкладка не найдена)
...
#### ParameterField.vue
Универсальный компонент поля параметра (название + полоска точек + опциональное пояснение), объединяющий `DotStrip`/`DotStripComplex` и логику пояснений в одном месте.

**Props:**
- `label: string`, `modelValue: number`, `max: number`
- `editable?: boolean` — редактируемое название
- `complex?: boolean` — использовать `DotStripComplex` вместо `DotStrip`
- `hasExplanation?: boolean`, `explanation?: string`, `disabled?: boolean`

> На данный момент компонент присутствует в кодовой базе, но не подключён ни к одной секции чар-листа — все секции реализуют аналогичную разметку (поле + `ExplanationTooltip`/`ExplanationModal` + `DotStrip`) напрямую в своих шаблонах.
...
| VII. Заметки | NotesSection.vue | Markdown-редактор: режим "Markdown" (исходник) и "Визуально" (WYSIWYG) |
...
- Опыт (ExperienceGrid)
- Урон (DotStripComplex, лимит = максимальное здоровье) + Богатство
- Увечья (InjuryFigure)
...
#### Кошмары и Бедламы
Кошмары и Бедламы — две независимые связанные линейки точек:
```
MAX кошмаров = 10 - Постоянная воля
MAX бедламов = 10 (фиксированный)
```
Нижняя линейка (Кошмары) хранит текущее число кошмаров. Верхняя линейка (Бедламы)
растёт только когда игрок повторно кликает по последней точке уже заполненной
до максимума линейки кошмаров (кошмар сверх лимита) — точка добавляется в
бедламы, а кошмары остаются на максимуме. Бедламы не уменьшаются при уменьшении
кошмаров, а изменение Постоянной воли не пересчитывает и не сбрасывает ни
кошмары, ни бедламы.
...
|----------|---------|----------|
| Атрибуты (социальные/ментальные/физические) | 0¹ | 6 |
| Постоянная воля | 2 | 10 |
| Перебросы | 0 | Постоянная воля |
| Кошмары | 0 | 10 - Постоянная воля |
| Бедламы | 0 | 10 |
| Опыт | 0 | 15 |
| Королевства (материк) | 0 | 5 |
| Королевства (фэйри) | 0 | 4 |
| Искусства, права рождения, проклятия, связи, особенности, артефакты, снаряжение | 0 | 5 |
| Расходники (обычные и магические) | 0 | 9 |

¹ Начальное значение атрибутов при создании персонажа — 1, но компонент `DotStrip` не запрещает уменьшить значение до 0 (минимум не проверяется в коде).
...
### Пустые поля при печати

`CollapsibleFields` скрывает незаполненные поля по умолчанию, но если на экране
нажать "Показать ещё" (`expanded = true`), они остаются видимыми - это только
экранное JS-состояние, к печати оно отношения не имеет. Поэтому каждая строка
такого поля в секциях (`AttributesSection`, `ArtsSection`, `ConnectionsSection`,
`InventorySection`, `CompanionsSection`) дополнительно помечена классом
`print:hidden`, если поле не заполнено (та же проверка `isUsed`/`isParamUsed`/
`isSpecUsed`/`isAbilityUsed`, что и у `CollapsibleFields`, но без привязки
к `expanded`) - при печати пустые поля скрыты всегда, вне зависимости
от того, что развёрнуто на экране.

### Компактная раскладка для печати

Печать нацелена на 3 страницы (референс - лист персонажа D&D): плотная сетка,
компактная типографика (6.5-13pt) и минимальные отступы вместо экранных
`p-4`/`gap-6`/`mb-4` и т.п. Секции группируются в 3 тематические страницы через
`print-break` в `TabContainer.vue`:
1. Основная информация + Атрибуты + Рассудок + Искусства
2. Связи + Инвентарь + Спутники
3. Заметки

Итоговое число страниц зависит от объёма заполненных данных (особенно длины
заметок) - три группы гарантируют начало каждой темы с новой страницы, но
сама группа может занять больше одной физической страницы при большом
количестве текста.

### Автоматическое сжатие, если группа не помещается на страницу

Все компактные print-размеры (шрифты, отступы, точки) выражены в `style.css`
как `calc(значение * var(--print-scale, 1))`. `src/utils/printFit.ts` по событию
`beforeprint` (навешивается в `App.vue`) находит те же группы страниц, что
и `print-break` в `TabContainer.vue` (по DOM, не по дублирующимся индексам),
измеряет фактическую высоту самой высокой группы и, если она не помещается
в печатную область A4 (см. `@page margin` в `style.css`), уменьшает
`--print-scale` (не ниже 0.6, дальше текст становится нечитаемым - лучше лишняя
страница, чем нечитаемый шрифт). После печати (`afterprint`) масштаб сбрасывается.

Расчёт делается за один проход (нужный масштаб = высота страницы / измеренная
высота), а не циклом с перемерами, поэтому точен настолько, насколько высота
контента линейно масштабируется вместе со шрифтом - в целом достаточно для
исправления небольших переполнений, но не заменяет собой саму компактную вёрстку.

**Важный нюанс `md:`-классов при печати.** Брейкпоинт `md:` (768px) при печати
может не срабатывать - ширина страницы A4 с полями (~1cm) даёт около 718px,
то есть ниже брейкпоинта. Поэтому все `md:`-классы, влияющие на раскладку
(`md:grid-cols-*`, `md:flex-nowrap`, `md:w-[N%]`), принудительно продублированы
в `style.css` внутри `@media print { @media (min-width: 0) { ... } }` -
без этого, например, пятиколоночный верхний блок (`MainSection.vue`, flex +
произвольные `%`-ширины) разваливается в печати в пять блоков на всю высоту
друг под другом. При добавлении новых `md:`-классов в разметку их нужно
так же продублировать здесь, иначе печать снова "расползётся".

### Поведение секций при печати

В обычном режиме `TabContainer.vue` показывает только одну активную секцию (`v-show`). При печати CSS принудительно переключает все `.tab-panel` в `display: block`, поэтому **на печать выводятся все семь секций подряд**, а не только открытая. Перед каждой секцией, кроме первой, класс `print-break` вставляет разрыв страницы (`page-break-before: always`), а скрытый в обычном режиме заголовок (`hidden print:block`) выводит название секции (например, «I. Атрибуты») в начале печатной страницы.
...
Workflow `.github/workflows/docker-build-publish.yml` при пуше в `master`/`main` и при тегах `v*`:
1. собирает Docker-образ и публикует его в GitHub Container Registry (`ghcr.io/<owner>/<repo>`), используя встроенный `GITHUB_TOKEN`;
2. (только для веток `master`/`main`) подключается по SSH к серверу, стягивает свежий образ и перезапускает контейнер.

Для шага деплоя нужно задать секреты репозитория (Settings → Secrets and variables → Actions):
- `DEPLOY_HOST` — адрес сервера
- `DEPLOY_USERNAME` — пользователь SSH
- `DEPLOY_SSH_KEY` — приватный SSH-ключ
- `DEPLOY_PORT` — порт SSH (опционально, по умолчанию 22)

Если пакет в GHCR приватный, его видимость нужно один раз переключить на нужную в настройках пакета на GitHub, либо оставить приватным — тогда логин на сервере через `GITHUB_TOKEN` (уже встроен в workflow) достаточен для `docker pull` в рамках того же запуска.
